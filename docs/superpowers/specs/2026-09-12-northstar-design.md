...
Revised a sixth time 2026-09-22 after the round-6 panel (astra, 8 findings, 6 confirmed and 2 partial by an
independent verifier; every round-5 numeric fix held). Root classes: **an acceptance that asserts `closed`
where the spec's own rule says `grey` is a check that cannot pass** (the ten random-mating null runs are
outside cards 1 and 4's signature; card state now has a precedence: grey wins, section 4), **a fixture the
page cannot load is an engine test wearing a smoke's name** (the page founds inside RUN at the slider bee,
fixed for the run, so the reach-switch stall becomes an engine test and the page's stall fixture is a pair
that stalls at `DEFAULT_BEE`, section 3), and **a probability table can quietly require the one change the
rule forbids** (chapter 5's 0.313 / 0.006 needed the mother-draw weight to move; the registered term now
acts inside the selfing test only and its table is 1 / 0.076 / 0.010 / 0.004). Also: M2 re-measures the
null block as well as the positives (no edge moves); the gap statistic counts placed plants; levels 2 and 6
say "no known win from placement alone"; chapter 8's packet counts carry their fixture; `antherTheta` wraps
to [-pi, pi]. Responses to all 8 findings at the end.

Design spec. Engine at master `ab13a90` (unchanged under `sim/` since `d18f972`; `git log
d18f972..HEAD -- sim/` is empty; `population.html` line numbers are `ab13a90`'s, shifted by the a11y commit `4502d69`, round 6). Every claim about current code carries a `file:line`; every
...
| fate                 | `sim/ibm.js:539-546`                                                                                                                    | `BOTH LOST` if extinct or fewer than 2 plants (`:540`); `HELD` if final ancestry variance > 0.4 x founding (`:543`); `one lost` if mean ancestry < 0.15 or > 0.85 (`:544`); else `FUSED` (`:545`). Canonical, not the only copy: the comment at `:525-532` says five experiments hold a copy; `grep -l "function fate(" experiments/*.js` finds eight (`experiments/secondary-contact.js:108` has no `BOTH LOST`) and the page has one (`population.html:1092-1099`); the sandbox calls `IBM.fateOf` |
...
| the page's run loop  | `population.html:768-827`                                                                                                               | `buildRun`: reads N, seed, generations, d, mating mode (`:718-722`); calls `I.foundTwoLineages` (`:727`); drives `I.step(pop, opts, rng, g, srng)` itself, no `brng`/`wrng`/`crng` (`:754-760`); stores `flowers`, `sites`, `sep = res.cluster.separation`, `ancVar` per generation (`:762-772`); returns `realised` (`:775`)                                                                                                                                                                        |
| loop fidelity test   | `tests/browser-bundle.test.js:157-200`                                                                                                  | "the page's own generation loop reproduces run() exactly"; it calls `I.step(pop, opts, rng, g, srng)` with `DEFAULTS` only (`:165,183`). It asserts a COPY of the loop with restarted streams (`const rng = E.makeRng(seed)` at `:177-178`, after founding) against `run({found})`, which restarts them too (`sim/ibm.js:2373-2376`); the page founds and steps on ONE `rng` (`population.html:776-778`). It does not test the page's protocol; M1 rewrites it (section 9, the protocol paragraph)                                                                                                                                                                                                                                                                                                                                                              |
| bundle               | `sim/browser-bundle.js`, built by `tools/build-browser-bundle.js:29-45`                                                                 | globals `Placement`, `Packing`, `Carryover`, `Evolve`, `Deception`, `IBM`; page aliases at `population.html:321-323`                                                                                                                                                                                                                                                                                                                                                                                 |
| 3D renderer          | `population.html:488` (ancestry tint), `:645` (pollen at the model's own `(s, phi)`), `:686` (scene dressing labelled as such)          | canvas `#field` (`:220`), drag to orbit. Draws the logged bout: `draw` clears the canvas (`:814`), reads `G.log` and returns if it is empty (`:826-828`). Under `phenology` the engine logs no bout, `log: !PH && opts.logBout && bi === 0 ? opts.logBout : null` (`sim/ibm.js:1438`, "THE SLICED PATH DOES NOT LOG", `:1432-1437`), so on levels 4 and 5 the field would be blank; M3b adds the no-log fallback (section 9)                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| body map             | `population.html:293-312`, `drawMap` ending at `:1023`                                                                                  | placements plotted on the animal, along the body vs around it                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| fate tile            | `population.html:281-288`, `fateOf` copy at `:1025-1032`, `show` at `:1034-1049`                                                        | tiles: generation, cluster separation, ancestry variance, fate                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
(`population.html:222-226`) at `siteN: 90` (`sim/ibm.js:600`); every experiment cited below ran
...
3. **Realised separation**, the body distance between the two lineages' anther sites, computed
   ...
   is NOT the "cluster separation" tile: that is `res.cluster.separation` (`population.html:818`),
...
4. **Run controls**, from `population.html:212-245`: seed, N, generations, mating mode
...
5. **The field and the body map**, as today (`population.html:248`, `:248`), plus the scrubber and
   ...
   **hybrid fraction** = share of `G.anc` strictly between 0.15 and 0.85
   (`experiments/hybrids-or-balance.js:64-65,87`; `anc` per generation at `population.html:793`)
...
The page prints "one lineage lost" today (`population.html:1097`) where the engine prints "one
...
that sets `reach` before founding founds nothing and throws in `gapOccupancy`. Round 6: that protocol
is the engine's, not the page's. The page founds inside RUN at the slider bee, fixed for the run
(`population.html:768-778`), so it cannot found at one reach and step at another; the reach switch is
the engine test of `stalled`, and **the page's stall fixture** is a pair that stalls at `DEFAULT_BEE`:
both lineages `Evolve.randomGenome(Evolve.makeRng(4))` with `antherT` set to 0.825, founded by M1's
recipe at seed 1 and the level configuration. Its founding placement is `{s: -0.159, phi: -1.306}`,
non-null, so the page founds; 24 of 30 founders carry a placement after the 0.02 spread; every one of
the 35 generations recruits 0 (verified). Its controls: the same pair at `antherT` 0.80 recruits 1050
and reads FUSED; at 0.85, the gene's bound, the placement is null and the page declines to found. That is
...
cards 1 and 4, the same seed under `randomMating` (`population.html:773`), which severs placement
...
`srng = I.signalRng(seed)` once, founds the pair with them (`population.html:776-778`) and drives
...
(`population.html:768-826`) into one file the page loads by `<script src>`, `population-run.js`
...
`rate > 0` (`:1844`); the page loop passes none (`population.html:805-811`). M3b builds the three
...
3. **The levels become a slideshow of findings.** If each level can only reproduce its finding,
   ...
   and levels 2 and 6 ship with no known win from placement alone and say so (level 6 lists the option
   wins of levels 4 and 5 as already known, section 5).
...
5. (round 4) There is ONE rng protocol, the page's: `rng` and `srng` are made once from the seed,
   found the pair and drive every generation (`population.html:776-778,754-760`). `run({found})`
...
self weight by the outcross grains landed, applied INSIDE the selfing test only: the mother selfs when
`rng() * weight[mother] < selfW[mother] / (1 + received[mother])`, with `weight[mother]` untouched.
Round 6: the round-5 table (0.313 at received 10, 0.006 at 90) was computed with `weight` recomputed
from the discounted `selfW` too, and `weight` is the mother-draw vector (`pick(..., weight)`, `:2130`),
so as numbered the row moved P(mother), which section 14 forbids and this chapter denies. With the
test alone changed, P(self) = (50 / (1 + received)) / (received + 50) runs 1 / 0.076 / 0.010 / 0.004
at received 0 / 10 / 50 / 90, against 1 / 0.833 / 0.500 / 0.357 untouched (verified): the floor is
unchanged at zero outcross, the assurance where it is needed, and the increment (0.833 to 0.076,
0.357 to 0.004) is the registered primary. It changes a non-zero
...
so a sectile load at `groom` 0.3 lost 14,780 grains and a pollinium 1,320 (fixture, round 6: the
flower `toFlower(randomGenome(makeRng(3)))`, `siteSet` of 60 at seeds 11 and 12, `runBout` with 400
visits at seed 7, `pollenPerFlower` 60, `visitsPerFlower` 25, `harvest` 0, `presentRate` 1, `massulae`
3 for the sectile load; M3b commits it as `tools/probe-packets.js` and its two counts as a test); `:144` says "resists
...
Astra's round-6 audit of `e50b665` (8 findings; 6 confirmed, 2 partial by an independent verifier; every
round-5 numeric fix held: both card controls, `edges <doc>` diff empty, `card23 rm8` 1 to 5, the reach
pair, the founding counts at N = 5 and 30). Line numbers are this revision's.

- **1** (BLOCKER, the smoke asserts `closed` where the signature rule says `grey`): fixed. Card state
  has a precedence, grey wins (section 4); M2's null block asserts `grey` and its STALLED run asserts
  `closed` on cards 1 and 4, `grey` on the rest; the "not that the card is closed" clause is gone.
- **2** (the page cannot found at one reach and step at another): fixed. The reach switch is an
  engine test; the page's stall fixture is the fixed-bee pair (`randomGenome(makeRng(4))`, `antherT`
  0.825, 0 recruits in 35 of 35, controls at 0.80 and 0.85) in section 3 and M2.
- **3** (`foundTwoLineages` burns draws, so the null block differs too; PARTIAL): fixed. M2
  re-measures the positives and the seeds 1 to 5 null block; no edge is recalibrated, and the row
  says why.
- **4** (chapter 5's 0.313 / 0.006 require moving the mother-draw weight): fixed. The term acts
  inside the selfing test only; the table is 1 / 0.076 / 0.010 / 0.004 and the round-5 table is
  named as the forbidden variant.
- **5** ("no known winning configuration" is false for level 6; PARTIAL): fixed. Levels 2 and 6
  say no known win from placement alone; the risk guard says the same.
- **6** ("10% of plants" is of placed plants): fixed on card 2 with the `[core, gap, null]` reading.
- **7** (14,780 / 1,320 have no repo producer): fixed. The fixture is written into chapter 8 and
  M3b commits it.
- **8** (`antherTheta` wraps to [-pi, pi], both endpoints): fixed in M1.
